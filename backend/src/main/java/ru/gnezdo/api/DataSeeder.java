...
public class DataSeeder implements ApplicationRunner {
    ...
    private static final String[] NAMES={"Алина","Максим","София","Артём","Ксения","Илья","Мария","Даниил","Виктория","Кирилл"};
    ...
    private void seedProfiles(){for(int i=0;i<50;i++){String name=NAMES[i%NAMES.length]+(i>=NAMES.length?" "+(i+1):"");jdbc.sql("insert into profiles(name,age,role,district,budget,distance,avatar,verified,bio,traits,red_flags) values (:name,:age,:role,:district,:budget,:distance,:avatar,:verified,:bio,:traits,:redFlags)").param("name",name).param("age",19+i%16).param("role",new String[]{"Дизайнер","Разработчик","Студент КубГУ","Маркетолог","Аналитик"}[i%5]).param("district",DISTRICTS[i%DISTRICTS.length]).param("budget",22000+i%7*4000).param("distance",BigDecimal.valueOf(.8+(i%13)*.8)).param("avatar","https://api.dicebear.com/9.x/notionists/svg?seed="+i).param("verified",i%4!=0).param("bio","Ищу уютное жильё в Краснодаре. Ценю честность, личные границы и понятные бытовые правила.").param("traits",tags(i,4)).param("redFlags",tags(i+5,2)).update();}}
